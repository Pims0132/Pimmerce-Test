...
const catColors: Record<string, string> = { 'Webdesign': '#0a0a0a', 'AI Marketing': '#3a3a3a', 'E-commerce': '#5a5a5a', 'Algemeen': '#8a8a8a' }
      ...
      <main style={{ background: '#f8f6f1', minHeight: '100vh' }}>
        <div style={{ background: '#fff', borderBottom: '1px solid rgba(10,10,10,.08)', padding: 'clamp(120px,16vh,180px) 6vw clamp(48px,6vh,80px)' }}>
          <div className="max-w-[1200px] mx-auto">
            ...
            <h1 style={{ fontFamily: 'Playfair Display, serif', fontWeight: 900, fontSize: 'clamp(2.8rem,6vw,6rem)', lineHeight: .88, letterSpacing: '-.04em', color: '#0a0a0a' }}>
              Tips & artikelen<br /><em style={{ fontStyle: 'italic', color: '#8a8a8a' }}>over digitale groei.</em>
            </h1>
          </div>
        </div>
        ...
        <div className="max-w-[1200px] mx-auto" style={{ padding: 'clamp(48px,6vh,80px) 6vw clamp(80px,12vh,140px)' }}>
          {blogs.length === 0 ? (
            ...
            <div style={{ display: 'grid', gap: 1, background: 'rgba(10,10,10,.06)', border: '1px solid rgba(10,10,10,.06)' }}>
              {blogs.map(b => (
                <Link key={b.id} href={`/kennisbank/${b.slug}`} style={{ textDecoration: 'none', display: 'block', background: '#fff', padding: 'clamp(24px,3.5vw,36px)', transition: 'background .2s' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#f8f6f1' }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <h2 style={{ fontFamily: 'Playfair Display, serif', fontWeight: 700, fontSize: 'clamp(1.1rem,2vw,1.5rem)', color: '#0a0a0a', letterSpacing: '-.02em', marginBottom: 8 }}>{b.titel}</h2>
                      {b.beschrijving && <p style={{ color: '#8a8a8a', fontSize: '.9rem', lineHeight: 1.65, fontWeight: 300 }}>{b.beschrijving}</p>}
                    </div>
